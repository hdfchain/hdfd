...
func hdfdMain() error {
	...
	hdfdLog.Infof("Version %s (Go version %s %s/%s)", version.String(),
		runtime.Version(), runtime.GOOS, runtime.GOARCH)
	hdfdLog.Infof("Home dir: %s", cfg.HomeDir)
	if cfg.NoFileLogging {
		hdfdLog.Info("File logging disabled")
	}
	...
	if cfg.MemProfile != "" {
		...
		if err != nil {
			hdfdLog.Errorf("Unable to create mem profile: %v", err)
			return err
		}
		defer f.Close()
	...
	svr, err := newServer(ctx, cfg.Listeners, db, cfg.params.Params,
		cfg.DataDir)
	if err != nil {
		hdfdLog.Errorf("Unable to start server: %v", err)
		return err
	}
