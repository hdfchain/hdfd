...
import (
	...
	"github.com/hdfchain/hdfd/limits"
)
...
// hdfdMain is the real main function for hdfd.  It is necessary to work around
// the fact that deferred functions do not run when os.Exit() is called.  The
// optional serverChan parameter is mainly used by the service code to be
// notified with the server once it is setup so it can gracefully stop it when
// requested from the service control manager.
func hdfdMain(serverChan chan<- *server) error {
	...
	defer hdfdLog.Info("Shutdown complete")

	// Show version and home dir at startup.
	hdfdLog.Infof("Version %s (Go version %s)", version(), runtime.Version())
	hdfdLog.Infof("Home dir: %s", cfg.HomeDir)

	// Enable http profiling server if requested.
	if cfg.Profile != "" {
		go func() {
			listenAddr := cfg.Profile
			hdfdLog.Infof("Creating profiling server "+
	...
	if cfg.CPUProfile != "" {
		...
		if err != nil {
			hdfdLog.Errorf("Unable to create cpu profile: %v", err.Error())
			return err
		}
	...
	if cfg.MemProfile != "" {
		...
		if err != nil {
			hdfdLog.Errorf("Unable to create cpu profile: %v", err)
			return err
		}
	...
	db, err := loadBlockDB()
	if err != nil {
		hdfdLog.Errorf("%v", err)
		return err
	}
	defer func() {
		// Ensure the database is sync'd and closed on shutdown.
		lifetimeNotifier.notifyShutdownEvent(lifetimeEventDBOpen)
		hdfdLog.Infof("Gracefully shutting down the database...")
		db.Close()
	}()
	...
	if err != nil {
		...
		hdfdLog.Errorf("Unable to start server on %v: %v",
			cfg.Listeners, err)
		return err
	}
	defer func() {
		lifetimeNotifier.notifyShutdownEvent(lifetimeEventP2PServer)
		hdfdLog.Infof("Gracefully shutting down the server...")
		server.Stop()
