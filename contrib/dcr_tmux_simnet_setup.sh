...
NODES_ROOT=${DCR_SIMNET_ROOT:-${HOME}/hdfdsimnetnodes}
RPCUSER="USER"
RPCPASS="PASS"
...
PRIMARY_DCRD_NAME=hdfd1
SECONDARY_DCRD_NAME=hdfd2
...
tmux send-keys "hdfd -C ../hdfd.conf --listen ${PRIMARY_DCRD_P2P} --miningaddr=${WALLET_MINING_ADDR}" C-m
...
#!/usr/bin/env bash
hdfctl -C ../hdfctl.conf -s ${SECONDARY_DCRD_RPC} "\$@"
EOF
...
tmux send-keys "hdfd -C ../hdfd.conf --listen ${SECONDARY_DCRD_P2P} --rpclisten ${SECONDARY_DCRD_RPC} --connect ${PRIMARY_DCRD_P2P}  --miningaddr=${WALLET_MINING_ADDR}" C-m
tmux select-pane -t 1
tmux send-keys "cd ${NODES_ROOT}/${SECONDARY_DCRD_NAME}" C-m
...
tmux send-keys "hdfwallet -C ../wallet.conf --rpcconnect=${SECONDARY_DCRD_RPC} --rpclisten=${SECONDARY_WALLET_RPC} --nogrpc" C-m
tmux select-pane -t 1
tmux send-keys "cd ${NODES_ROOT}/${SECONDARY_WALLET_NAME}" C-m
