...
caller to specify things such as the user agent name and version, the hdfchain
network to use, which services it supports, and callbacks to invoke when hdfchain
...
In order to do anything useful with a peer, it is necessary to react to hdfchain
...
For convenience, a callback hook for all of the currently supported hdfchain
...
(https://godoc.org/github.com/hdfchain/hdfd/wire#hdr-Bitcoin_Improvement_Proposals)
*/
package peer
