...
example, the DebugLevel call is an extension only provided by hdfd (and
hdfwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that hdfd intentionally separates the wallet
functionality into a separate process named hdfwallet.  This means if you are
connected to the hdfd RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, hdfwallet provides pass through treatment
...
The following full-blown client examples are in the examples directory:
 ...
 - hdfdwebsockets
   ...
   block count
 - hdfwalletwebsockets
   Connects to a hdfwallet RPC server using TLS-secured websockets, registers
