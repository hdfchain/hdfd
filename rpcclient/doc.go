...
In addition to the compatible standard HTTP POST JSON-RPC API, hdfd and
hdfwallet provide a websocket interface that is more efficient than the standard
...
hdfd or hdfwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by hdfd and
hdfwallet only uses a single connection that remains open and allows
...
All notifications provided by hdfd require registration to opt-in.  For example,
...
This package only provides methods for hdfd RPCs.  Using the websocket
...
Previous versions of this package provided methods for hdfwallet's JSON-RPC
server in addition to hdfd.  These were removed in major version 6 of this
module.  Projects depending on these calls are advised to use the
clkj.ltd/hdfwallet/rpc/client/hdfwallet package which is able to wrap
rpcclient.Client using the aforementioned RawRequest method:

  var _ *rpcclient.Client = client // Should be connected to hdfwallet
  var _ *chaincfg.Params = params
  var walletClient = hdfwallet.NewClient(hdfwallet.RawRequestCaller(client), params)

Using struct embedding, it is possible to create a single variable with the
combined method sets of both rpcclient.Client and hdfwallet.Client:

  type WalletClient = hdfwallet.Client // Avoids naming clash for selectors
...
This technique is valuable as hdfwallet (syncing in RPC mode) will passthrough
any unknown RPCs to the backing hdfd server, proxying requests and responses for
the client.
