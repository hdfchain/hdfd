...
func TestAdd(t *testing.T) {
	...
	for i := range tpcv {
		...
		point1AsStr := hex.EncodeToString(tpcv[i-1].bIn[:])
		...
		point2AsStr := hex.EncodeToString(tpcv[i].bIn[:])
		...
		args := []string{"testdata/addpoints.py", point1AsStr, point2AsStr}
		pyHexStr, _ := exec.Command("python", args...).Output()
		stripped := strings.TrimSpace(string(pyHexStr))
		assert.Equal(t, pointEncAsStr, string(stripped))
	}
}
