...
// MessageHeaderSize is the number of bytes in a Hdfchain message header.
// Hdfchain network (magic) 4 bytes + command 12 bytes + payload length 4 bytes +
// checksum 4 bytes.
const MessageHeaderSize = 24

// CommandSize is the fixed size of all commands in the common Hdfchain message
...
// Message is an interface that describes a Hdfchain message.  A type that
...
// messageHeader defines the header structure for all Hdfchain protocol messages.
...
// readMessageHeader reads a Hdfchain message header from r.
...
// WriteMessageN writes a Hdfchain Message to w including the necessary header
...
// WriteMessage writes a Hdfchain Message to w including the necessary header
...
// ReadMessageN reads, validates, and parses the next Hdfchain Message from r for
// the provided protocol version and Hdfchain network.  It returns the number of
...
func ReadMessageN(r io.Reader, pver uint32, dcrnet CurrencyNet) (int, Message, []byte, error) {
	...
	// Check for messages from the wrong Hdfchain network.
	if hdr.magic != dcrnet {
...
// ReadMessage reads, validates, and parses the next Hdfchain Message from r for
// the provided protocol version and Hdfchain network.  It returns the parsed
