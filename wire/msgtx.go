...
// this msgTx is. You can use a bitmask for this too, but Hdfchain just splits
...
// OutPoint defines a hdfchain data type that is used to track previous
// transaction outputs.
...
// NewOutPoint returns a new hdfchain transaction outpoint point with the
...
// NewTxIn returns a new hdfchain transaction input with the provided
...
// NewTxOut returns a new hdfchain transaction output with the provided
...
// MsgTx implements the Message interface and represents a hdfchain tx message.
...
func writeTxScriptsToMsgTx(msg *MsgTx, totalScriptSize uint64, mType TxSerializeType) {
	...
	//
	// Closures around writing the TxIn and TxOut scripts are used in Hdfchain
...
// LegacyBtcDecode decodes r using the hdfchain protocol encoding into the
...
// in that BtcDecode decodes from the Hdfchain wire protocol as it was sent
...
// LegacyBtcEncode encodes the receiver to w using the Hdfchain protocol encoding.
...
// encodes the transaction to the hdfchain wire protocol in order to be sent
// across the network.  The wire encoding can technically differ depending on
// the protocol version and doesn't even really need to match the format of a
// stored transaction at all.  As of the time this comment was written, the
// encoded transaction is the same in both instances, but there is a distinct
...
// LegacySerialize encodes the transaction in hdfchain legacy format, for
// compatibility.
...
// appropriate transaction output entry. This is for legacy hdfchain format.
...
// NewMsgTx returns a new hdfchain tx message that conforms to the Message
...
// WriteOutPoint encodes op to the hdfchain protocol encoding for an OutPoint
// to w.
...
// legacyWriteOutPoint encodes op to the hdfchain protocol encoding for a legacy
// Hdfchain OutPoint to w.
...
// legacyWriteTxIn encodes ti to the hdfchain protocol encoding for a transaction
// input (TxIn) to w for hdfchain legacy format.
...
// writeTxInPrefixs encodes ti to the hdfchain protocol encoding for a transaction
...
// writeTxWitness encodes ti to the hdfchain protocol encoding for a transaction
...
// writeTxInWitnessSigning encodes ti to the hdfchain protocol encoding for a
...
// writeTxInWitnessValueSigning encodes ti to the hdfchain protocol encoding for a
...
// writeTxOut encodes to into the hdfchain protocol encoding for a transaction
// output (TxOut) to w.
...
// (TxOut) in legacy Hdfchain format (for tests).
...
// legacyWriteTxOut encodes to into the hdfchain protocol encoding for a transaction
// output (TxOut) to w in legacy Hdfchain format (for tests).
