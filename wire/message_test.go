...
func makeHeader(dcrnet CurrencyNet, command string,
	...
	// The length of a Hdfchain message header is 24 bytes.
	// 4 byte magic number of the Hdfchain network + 12 byte command + 4 byte
...
func TestReadMessageWireErrors(t *testing.T) {
	...
	tests := []struct {
		...
		pver    uint32      // Protocol version for wire encoding
		dcrnet  CurrencyNet // Hdfchain network for wire encoding
