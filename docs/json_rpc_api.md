...
hdfd provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that is
fully compatible with the original bitcoind/bitcoin-qt.  There are a few key
differences between hdfd and bitcoind as far as how RPCs are serviced:
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  which leads to several issues, hdfd intentionally splits the wallet and chain
  ...
  talking directly to hdfd, only chain-related RPCs are available.  However both
  chain-related and wallet-related RPCs are available via
  [dcrwallet](https://github.com/hdfchain/dcrwallet).
* hdfd is secure by default which means that the RPC connection is TLS-enabled
  by default
* hdfd provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for hdfd RPC and are used by applications
such as [dcrwallet](https://github.com/hdfchain/dcrwallet) for inter-process
communication with hdfd.  The websocket connection endpoint for hdfd is
...
The hdfd RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
hdfd is `wss://your_ip_or_domain:9109/ws`.
...
to a hdfd RPC server:

* **rpcuser** is the full-access username configured for the hdfd RPC server
* **rpcpass** is the full-access password configured for the hdfd RPC server
* **rpclimituser** is the limited username configured for the hdfd RPC server
* **rpclimitpass** is the limited password configured for the hdfd RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the hdfd
  server is configured with.  It is automatically generated by hdfd and placed
  in the hdfd home directory (which is typically `%LOCALAPPDATA%\Dcrd` on
  Windows and `~/.hdfd` on POSIX-like OSes)

**NOTE:** As mentioned above, hdfd is secure by default which means the RPC
...
The hdfd RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
hdfd comes with a separate utility named `dcrctl` which can be used to issue
these RPC commands via HTTP POST requests to hdfd after configuring it with the
...
API compatible with the original hdfd client.
...
|23|[getwork](#getwork)|N|Returns formatted hash data to work on or checks and submits solved data.<br /><br />NOTE: Since hdfd does not have the wallet integrated to provide payment addresses, hdfd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
...
|26|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><br />NOTE: hdfd does not yet implement the `allowhighfees` parameter, so it has no effect|
|27|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since hdfd does not have the wallet integrated to provide payment addresses, hdfd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|28|[stop](#stop)|N|Shutdown hdfd.|
|29|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|30|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since hdfd does not have a wallet integrated, hdfd will only return whether the address is valid or not.|
...
|34|[getcurrentnet](#getcurrentnet)|Y|Get hdfchain network hdfd is running on.|
...
|Parameters|1. `transaction inputs`:  `(JSON array, required)` json array of json objects.<br />`hash`: `(string, required)` the hash of the input.</br> `vout`: (numeric, required) the specific output of the input transaction to redeem transaction.<br /><br />`[{"txid": "hash", "vout": n}, ...]`<br /><br />2. `addresses and amounts`: `(JSON object, required)` - json object with addresses as keys and amounts as values.</br>`address`:   `(numeric, required)` the address to send to as the key and the amount in HDF as the value.<br /><br />`{"address": n.nnn, ...}`|
...
|Example Return|`[{"addr": "178.172.xxx.xxx:9108", "services": "00000001", "lastrecv": 1388183523, "lastsend": 1388185470, "bytessent": 287592965, "bytesrecv": 780340, "conntime": 1388182973, "pingtime": 405551, "pingwait": 183023, "version": 70001, "subver": "/hdfd:0.4.0/", "inbound": false, "startingheight": 276921, "currentheight": 276955, "syncnode": true }, ...]`|
[Return to Overview](#MethodOverview)<br />
...
|Description|Returns information about a transaction given its hash.|
|Notes|Since hdfd does not have the wallet integrated to provide payment addresses, hdfd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.
...
|Notes|NOTE: Since hdfd does not have the wallet integrated to provide payment addresses, hdfd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|hdfd does not yet implement the `allowhighfees` parameter, so it has no effect.|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown hdfd.|
|Returns|`"hdfd stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Parameters|1. `address`: `(string, required)` hdfchain address.|
|Description|Verify an address is valid.|
|Returns|`(json object)`<br />`isvalid`: `(bool)` whether or not the address is valid.<br />`address`: `(string)` the hdfchain address validated.<br /><br />`{"isvalid": true or false,"address": "hdfchainaddress"}`|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database. The actual checks performed by the `checklevel` parameter is implementation specific. <br /><br />For hdfd this is: <br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|Dcrd currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for hdfd.|
...
|Description|Dynamically changes the debug logging level.<br />The levelspec can either a debug level or of the form `<subsystem>=<level>,<subsystem2>=<level2>,...`<br />The valid debug levels are `trace`, `debug`, `info`, `warn`, `error`, and `critical`.<br />The valid subsystems are `AMGR`, `ADXR`, `BCDB`, `BMGR`, `HDFD`, `CHAN`, `DISC`, `PEER`, `RPCS`, `SCRP`, `SRVR`, and `TXMP`.<br />Additionally, the special keyword `show` can be used to get a list of the available subsystems.|
|Returns|`string`|
|Example Return|`Done.`|
|Example `show` Return|`Supported subsystems [AMGR ADXR BCDB BMGR HDFD CHAN DISC PEER RPCS SCRP SRVR TXMP]`|
...
|Parameters|None|
|Description|Get hdfchain network hdfd is running on.|
|Returns|`numeric`|
...
|Parameters|1. `Addresses`: `(JSON array, required)`<br />`hdfchainaddress`: `(string)` the hdfchain address.<br /><br />`["hdfchainaddress", ...]`|
...
|Description|Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this hdfd instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSMethodOverview)<br />
...
hdfd uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll hdfd for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
[dcrrpcclient](https://github.com/hdfchain/dcrrpcclient) package.
...
[dcrrpcclient](https://github.com/hdfchain/dcrrpcclient) package to connect with
a hdfd instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by hdfd when it starts the RPC server and doesn't already
	// have one.
...
The following is an example Go application which uses the
[dcrrpcclient](https://github.com/hdfchain/dcrrpcclient) package to connect with
a hdfd instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[dcrrpcclient](https://github.com/hdfchain/dcrrpcclient) package to connect with
a hdfd instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a hdfd instance,
...
// generated by hdfd when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/hdfd/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The hdfd generated certificate acts as
