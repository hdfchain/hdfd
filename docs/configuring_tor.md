...
hdfd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, hdfd supports a hybrid,
...
While it is easier to only run as a client, it is more beneficial to the Hdfchain
...
Configuring hdfd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the hdfd command
line or in the hdfd configuration file.  Typically the Tor proxy address will be
...
By default, hdfd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since hdfd will act as the server.
...
```bash
$ ./hdfd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
hdfd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the hdfd command line or in the hdfd
...
```bash
$ ./hdfd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
hdfd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./hdfd --proxy=127.0.0.1:9050 --torisolation
```
