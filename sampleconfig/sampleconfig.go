...
; The default is ~/.hdfd/data on POSIX OSes, $LOCALAPPDATA/Hdfd/data on Windows,
; ~/Library/Application Support/Hdfd/data on macOS, and $homed/hdfd/data on
; Plan9.  Environment variables are expanded so they may be used.  NOTE: Windows
; environment variables are typically %VARIABLE%, but they must be accessed with
; $VARIABLE here.
; datadir=~/.hdfd/data                            ; Unix
; datadir=$LOCALAPPDATA/Hdfd/data                 ; Windows
; datadir=~/Library/Application Support/Hdfd/data ; macOS
...
; line.  hdfd will not contact 3rd-party sites to obtain external ip addresses.
...
; with, but the behavior is slightly different.  By default, hdfd will query DNS
...
; Disable DNS seeding for peers.  By default, when hdfd starts, it will use
...
; which is used to control and query information from a running hdfd process.
;
...
// HdfctlSampleConfig is a string containing the commented example config for hdfctl.
const HdfctlSampleConfig = `[Application Options]
