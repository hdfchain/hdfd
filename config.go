...
type config struct {
	...
	AltDNSNames          []string `long:"altdnsnames" description:"Specify additional DNS names to use when generating the RPC server certificate" env:"HDFD_ALT_DNSNAMES" env-delim:","`
	...
	MinRelayTxFee    float64 `long:"minrelaytxfee" description:"The minimum transaction fee in HDF/kB to be considered a non-zero fee"`
...
func loadConfig() (*config, []string, error) {
	...
	for _, oldDir := range oldTestNets {
		if fileExists(oldDir) {
			hdfdLog.Warnf("Block chain data from previous testnet"+
	...
	if configFileError != nil {
		hdfdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// hdfdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func hdfdDial(ctx context.Context, network, addr string) (net.Conn, error) {
...
// hdfdLookup returns the correct DNS lookup function to use depending on the
...
func hdfdLookup(host string) ([]net.IP, error) {
