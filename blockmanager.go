...
import (
	...
	peerpkg "github.com/hdfchain/hdfd/peer/v2"
...
func removeRegressionDB(dbPath string) error {
	...
	if err == nil {
		hdfdLog.Infof("Removing regression test database from '%s'", dbPath)
		if fi.IsDir() {
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		hdfdLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB(params *chaincfg.Params) (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		hdfdLog.Infof("Creating block database in memory.")
	...
	hdfdLog.Infof("Loading block database from '%s'", dbPath)
	...
	hdfdLog.Info("Block database loaded")
	return db, nil
}
