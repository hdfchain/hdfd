...
import (
	...
	"github.com/hdfchain/hdfd/blockchain/v3"
	"github.com/hdfchain/hdfd/blockchain/v3/indexers"
	...
	"github.com/hdfchain/hdfd/gcs/v2/blockcf"
	"github.com/hdfchain/hdfd/internal/fees"
	"github.com/hdfchain/hdfd/internal/mempool"
	"github.com/hdfchain/hdfd/internal/mining"
	"github.com/hdfchain/hdfd/internal/mining/cpuminer"
	"github.com/hdfchain/hdfd/internal/rpcserver"
	"github.com/hdfchain/hdfd/internal/version"
	"github.com/hdfchain/hdfd/lru"
	"github.com/hdfchain/hdfd/peer/v2"
...
func (s *server) querySeeders(ctx context.Context) {
	...
	for _, seeder := range seeders {
		go func(seeder string) {
			...
			addrs, err := connmgr.SeedAddrs(ctx, seeder, hdfdDial,
...
// hdfchain network type specified by chainParams.  Use start to begin accepting
...
func newServer(ctx context.Context, listenAddrs []string, db database.DB, chainParams *chaincfg.Params, dataDir string) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, hdfdLookup)
...
func addrStringToNetAddr(addr string) (net.Addr, error) {
	...
	// The hdfdLookup function will transparently handle performing the
	// lookup over Tor if necessary.
	ips, err := hdfdLookup(host)
	if err != nil {
		return nil, err
	}
	if len(ips) == 0 {
