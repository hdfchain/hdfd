...
const (
	...
	// HDF/kB, this results in a maximum allowed high fee of 1 HDF/kB.
...
type Policy struct {
	...
	// MinRelayTxFee defines the minimum transaction fee in HDF/kB to be
...
func (mp *TxPool) checkVoteDoubleSpend(vote *dcrutil.Tx) error {
	...
	for _, existingVote := range mp.votes[hashVotedOn] {
		if existingVote.TicketHash == ticketSpent {
			...
			//
			// TODO(hdfchain): Ideally the votes map and mempool would be kept in
