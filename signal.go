...
func shutdownListener() context.Context {
	...
	go func() {
		...
		case sig := <-interruptChannel:
			hdfdLog.Infof("Received signal (%s).  Shutting down...", sig)

		case <-shutdownRequestChannel:
			hdfdLog.Infof("Shutdown requested.  Shutting down...")
		}
		cancel()
		...
		for {
			select {
			case sig := <-interruptChannel:
				hdfdLog.Infof("Received signal (%s).  Already "+
					"shutting down...", sig)

			case <-shutdownRequestChannel:
				hdfdLog.Info("Shutdown requested.  Already " +
					"shutting down...")
			}
		}
	}()
