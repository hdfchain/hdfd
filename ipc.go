...
// processes to communicate with and and manage the lifetime of a hdfd child
...
// message, shutdown begins.  This prevents hdfd from continuing to run
...
func serviceControlPipeRx(fd uintptr) {
	...
	for {
		...
		if err != nil {
			hdfdLog.Errorf("Failed to read from pipe: %v", err)
			break
		}
	}
...
// for a child hdfd process to communicate with a parent process without the
...
func serviceControlPipeTx(fd uintptr) {
	...
	hdfdLog.Errorf("Failed to write to pipe: %v", err)
}
