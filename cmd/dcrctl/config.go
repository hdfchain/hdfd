...
func createDefaultConfigFile(destinationPath string) error {
	...
	hdfdConfigPath := filepath.Join(hdfdHomeDir, "hdfd.conf")
	if !fileExists(hdfdConfigPath) {
		return nil
	}

	// Read hdfd.conf from its default path
	hdfdConfigFile, err := os.Open(hdfdConfigPath)
	if err != nil {
		return err
	}
	defer hdfdConfigFile.Close()
	content, err := ioutil.ReadAll(hdfdConfigFile)
	if err != nil {
		return err
	}
