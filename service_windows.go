...
const (
	// svcName is the name of hdfd service.
	svcName = "hdfdsvc"
	...
	svcDesc = "Downloads and stays synchronized with the hdfchain block " +
...
// logServiceStartOfDay logs information about hdfd when the main server has
...
// hdfdService houses the main service handler which handles all service
// updates and launching hdfdMain.
type hdfdService struct{}
...
// long-running hdfdMain (which is the real meat of hdfd), handles service
// change requests, and notifies the service control manager of changes.
func (s *hdfdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start hdfdMain in a separate goroutine so the service can start
...
// installService attempts to install the hdfd service.  Typically this should
...
// removeService attempts to uninstall the hdfd service.  Typically this should
...
// startService attempts to start the hdfd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &hdfdService{})
	if err != nil {
