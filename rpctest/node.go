...
import (
	...
	rpc "github.com/hdfchain/hdfd/rpcclient/v2"
)

// nodeConfig contains all the args, and data required to launch a hdfd process
...
// arguments returns an array of arguments that be used to launch the hdfd
// process.
...
// command returns the exec.Cmd which will be used to start the hdfd process.
...
// to the hdfd process that is launched via Start().
...
// as the base for the log and data directories for hdfd.
...
// start creates a new hdfd process, and writes its pid in a file reserved for
...
func (n *node) start() error {
	...
	pid, err := os.Create(filepath.Join(n.config.String(), "hdfd.pid"))
	if err != nil {
		return err
	}
...
// stop interrupts the running hdfd process process, and waits until it exits
...
// shutdown terminates the running hdfd process, and cleans up all
