...
import (
	...
	"github.com/hdfchain/hdfd/rpcclient/v6"
...
// Harness fully encapsulates an active hdfd process to provide a unified
// platform for creating rpc driven integration tests involving hdfd. The
// active hdfd node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active hdfd process is fully
...
func New(t *testing.T, activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers, extraArgs []string) (*Harness, error) {
	...
	// Uncomment and change to enable additional hdfd debug/trace output.
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the hdfd node itself. This spawns a new process which will be
	// managed
	...
	// Ensure hdfd properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created hdfd
